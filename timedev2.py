...
def caldW(net, inputs, eloc, delta,tole):
    sample_num = inputs.shape[0]
    ...
    dw = delta*x
    sigma = np.matmul(Skk.real.conj(),x) - Fk.imag
    return dw,x,sigma

def adaptivetime(delta,dw,tole_2):
    new_delta = delta*(tole_2/dw)**(0.33)
    return new_delta

def NewtonMethod(net, inputs, eloc, delta,tole):
    dw,x,sigma = caldW(net, inputs, eloc, delta,tole)
